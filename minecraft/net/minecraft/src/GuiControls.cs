namespace net.minecraft.src
{
	public class GuiControls : GuiScreen
	{
		...
		protected internal string screenTitle = "Controls";
		...
		protected internal override void actionPerformed(GuiButton guiButton1)
		{
			...
			{
				((GuiButton)this.controlList[i2]).displayString = this.options.getOptionDisplayString(i2);
			}
		...
		protected internal override void mouseClicked(int i1, int i2, int i3)
		{
			if (this.buttonId >= 0)
			{
				...
				((GuiButton)this.controlList[this.buttonId]).displayString = this.options.getOptionDisplayString(this.buttonId);
				this.buttonId = -1;
		...
		public override void drawScreen(int i1, int i2, float f3)
		{
			...
			for (int i5 = 0; i5 < this.options.keyBindings.Length; ++i5)
			{
				...
				for (int i7 = 0; i7 < this.options.keyBindings.Length; ++i7)
				{
					if (i7 != i5 && this.options.keyBindings[i5].keyCode == this.options.keyBindings[i7].keyCode)
					{
						z6 = true;
						break;
					}
				...
				if (this.buttonId == i5)
				{
					((GuiButton)this.controlList[i5]).displayString = "\u00a7f> \u00a7e??? \u00a7f<";
				}
				else if (z6)
				{
					((GuiButton)this.controlList[i5]).displayString = "\u00a7c" + this.options.getOptionDisplayString(i5);
				}
				else
				{
					((GuiButton)this.controlList[i5]).displayString = this.options.getOptionDisplayString(i5);
				}
