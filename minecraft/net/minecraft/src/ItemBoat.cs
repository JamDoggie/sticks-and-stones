    ...
    public class ItemBoat : Item
	{
		...
		public override ItemStack onItemRightClick(ItemStack itemStack1, World world2, EntityPlayer entityPlayer3)
		{
			...
			else
			{
				...
				bool z26 = false;
				float f27 = 1.0F;
				System.Collections.IList list28 = world2.getEntitiesWithinAABBExcludingEntity(entityPlayer3, entityPlayer3.boundingBox.addCoord(vec3D25.xCoord * d21, vec3D25.yCoord * d21, vec3D25.zCoord * d21).expand((double)f27, (double)f27, (double)f27));

				for (int i29 = 0; i29 < list28.Count; ++i29)
				{
					Entity entity30 = (Entity)list28[i29];
					if (entity30.canBeCollidedWith())
					{
						float f31 = entity30.CollisionBorderSize;
