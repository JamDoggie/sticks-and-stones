...
namespace net.minecraft.src
{
    ...
    public class BlockVine : Block
	{
		...
		private bool canBePlacedOn(int i1)
		{
			if (i1 == 0)
			{
				return false;
		...
		private bool canVineStay(World world1, int i2, int i3, int i4)
		{
		...
		{
			return iBlockAccess1.getBiomeGenForCoords(i2, i4).BiomeFoliageColor;
		}
		...
		{
			if (!world1.isRemote && world1.rand.Next(4) == 0)
			{
				sbyte b6 = 4;
				int i7 = 5;
				bool z8 = false;
				...
				else
				{
					...
					if (i10 >= 2 && i10 <= 5 && (i9 & 1 << i11) == 0)
					{
						...
						{
							if (Block.blocksList[i12].blockMaterial.Opaque && Block.blocksList[i12].renderAsNormalBlock())
							{
