...
namespace net.minecraft.src
{
    ...
    public class BlockFurnace : BlockContainer
	{
		...
		protected internal BlockFurnace(int i1, bool z2) : base(i1, Material.rock)
		{
			this.isActive = z2;
		...
		public override void randomDisplayTick(World world1, int i2, int i3, int i4, RandomExtended random5)
		{
			if (this.isActive)
			{
				...
				float f8 = (float)i3 + 0.0F + random5.NextSingle() * 6.0F / 16.0F;
				float f9 = (float)i4 + 0.5F;
				float f10 = 0.52F;
				float f11 = random5.NextSingle() * 0.6F - 0.3F;
				if (i6 == 4)
				{
		...
		public static void updateFurnaceBlockState(bool z0, World world1, int i2, int i3, int i4)
		{
		...
		public override void onBlockRemoval(World world1, int i2, int i3, int i4)
		{
			if (!keepFurnaceInventory)
			{
				...
				{
					for (int i6 = 0; i6 < tileEntityFurnace5.SizeInventory; ++i6)
					{
						ItemStack itemStack7 = tileEntityFurnace5.getStackInSlot(i6);
						if (itemStack7 != null)
						{
							float f8 = this.furnaceRand.NextSingle() * 0.8F + 0.1F;
							float f9 = this.furnaceRand.NextSingle() * 0.8F + 0.1F;
							float f10 = this.furnaceRand.NextSingle() * 0.8F + 0.1F;

							while (itemStack7.stackSize > 0)
							{
								int i11 = this.furnaceRand.Next(21) + 10;
