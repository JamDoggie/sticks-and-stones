using BlockByBlock;
using SixLabors.ImageSharp;
using SixLabors.ImageSharp.PixelFormats;
using System;

namespace net.minecraft.src
{

	public class TerrainTextureManager
	{
		private float[] texCols = new float[768];
		private int[] pixels = new int[17408];
		private int[] zBuf = new int[17408];
		private int[] waterBuf = new int[17408];
		private int[] waterBr = new int[17408];
		private int[] yBuf = new int[34];
		private int[] textures = new int[768];

		public TerrainTextureManager()
		{
			try
			{
				Image<Bgra32> bufferedImage1 = Image.Load<Bgra32>(GameEnv.GetResourceAsStream("/terrain.png"));
				int[] i2 = new int[65536];
				TextureManager.FillIntBufferWithImage(bufferedImage1, i2);

				for (int i3 = 0; i3 < 256; ++i3)
				{
					int i4 = 0;
					int i5 = 0;
					int i6 = 0;
					int i7 = i3 % 16 * 16;
					int i8 = i3 / 16 * 16;
					int i9 = 0;

					for (int i10 = 0; i10 < 16; ++i10)
					{
						for (int i11 = 0; i11 < 16; ++i11)
						{
							int i12 = i2[i11 + i7 + (i10 + i8) * 256];
							int i13 = i12 >> 24 & 255;
							if (i13 > 128)
							{
								i4 += i12 >> 16 & 255;
								i5 += i12 >> 8 & 255;
								i6 += i12 & 255;
								++i9;
							}
						}

						if (i9 == 0)
						{
							++i9;
						}

						this.texCols[i3 * 3 + 0] = (float)(i4 / i9);
						this.texCols[i3 * 3 + 1] = (float)(i5 / i9);
						this.texCols[i3 * 3 + 2] = (float)(i6 / i9);
					}
				}
			}
			catch (IOException iOException14)
			{
				Console.WriteLine(iOException14.ToString());
				Console.Write(iOException14.StackTrace);
			}

			for (int i15 = 0; i15 < 4096; ++i15)
			{
				if (Block.blocksList[i15] != null)
				{
					this.textures[i15 * 3 + 0] = Block.blocksList[i15].getBlockTextureFromSide(1);
					this.textures[i15 * 3 + 1] = Block.blocksList[i15].getBlockTextureFromSide(2);
					this.textures[i15 * 3 + 2] = Block.blocksList[i15].getBlockTextureFromSide(3);
				}
			}

		}

		public virtual void render(IsoImageBuffer isoImageBuffer1)
		{
			World world2 = isoImageBuffer1.level;
			if (world2 == null)
			{
				isoImageBuffer1.noContent = true;
				isoImageBuffer1.rendered = true;
			}
			else
			{
				int i3 = isoImageBuffer1.x * 16;
				int i4 = isoImageBuffer1.y * 16;
				int i5 = i3 + 16;
				int i6 = i4 + 16;
				Chunk chunk7 = world2.getChunkFromChunkCoords(isoImageBuffer1.x, isoImageBuffer1.y);
				if (chunk7.Empty)
				{
					isoImageBuffer1.noContent = true;
					isoImageBuffer1.rendered = true;
				}
				else
				{
					isoImageBuffer1.noContent = false;
					Arrays.Fill(this.zBuf, 0);
					Arrays.Fill(this.waterBuf, 0);
					Arrays.Fill(this.yBuf, 544);

					for (int i8 = i6 - 1; i8 >= i4; --i8)
					{
						for (int i9 = i5 - 1; i9 >= i3; --i9)
						{
							int i10 = i9 - i3;
							int i11 = i8 - i4;
							int i12 = i10 + i11;
							bool z13 = true;

							for (int i14 = 0; i14 < 128; ++i14)
							{
								int i15 = i11 - i10 - i14 + 544 - 16;
								if (i15 < this.yBuf[i12] || i15 < this.yBuf[i12 + 1])
								{
									Block block16 = Block.blocksList[world2.getBlockId(i9, i14, i8)];
									if (block16 == null)
									{
										z13 = false;
									}
									else if (block16.blockMaterial == Material.water)
									{
										int i24 = world2.getBlockId(i9, i14 + 1, i8);
										if (i24 == 0 || Block.blocksList[i24].blockMaterial != Material.water)
										{
											float f25 = (float)i14 / 127.0F * 0.6F + 0.4F;
											float f26 = world2.getLightBrightness(i9, i14 + 1, i8) * f25;
											if (i15 >= 0 && i15 < 544)
											{
												int i27 = i12 + i15 * 32;
												if (i12 >= 0 && i12 <= 32 && this.waterBuf[i27] <= i14)
												{
													this.waterBuf[i27] = i14;
													this.waterBr[i27] = (int)(f26 * 127.0F);
												}

												if (i12 >= -1 && i12 <= 31 && this.waterBuf[i27 + 1] <= i14)
												{
													this.waterBuf[i27 + 1] = i14;
													this.waterBr[i27 + 1] = (int)(f26 * 127.0F);
												}

												z13 = false;
											}
										}
									}
									else
									{
										if (z13)
										{
											if (i15 < this.yBuf[i12])
											{
												this.yBuf[i12] = i15;
											}

											if (i15 < this.yBuf[i12 + 1])
											{
												this.yBuf[i12 + 1] = i15;
											}
										}

										float f17 = (float)i14 / 127.0F * 0.6F + 0.4F;
										int i18;
										int i19;
										float f20;
										float f22;
										if (i15 >= 0 && i15 < 544)
										{
											i18 = i12 + i15 * 32;
											i19 = this.textures[block16.blockID * 3 + 0];
											f20 = (world2.getLightBrightness(i9, i14 + 1, i8) * 0.8F + 0.2F) * f17;
											if (i12 >= 0 && this.zBuf[i18] <= i14)
											{
												this.zBuf[i18] = i14;
												this.pixels[i18] = unchecked((int)0xFF000000) | (int)(this.texCols[i19 * 3 + 0] * f20) << 16 | (int)(this.texCols[i19 * 3 + 1] * f20) << 8 | (int)(this.texCols[i19 * 3 + 2] * f20);
											}

											if (i12 < 31)
											{
												f22 = f20 * 0.9F;
												if (this.zBuf[i18 + 1] <= i14)
												{
													this.zBuf[i18 + 1] = i14;
													this.pixels[i18 + 1] = unchecked((int)0xFF000000) | (int)(this.texCols[i19 * 3 + 0] * f22) << 16 | (int)(this.texCols[i19 * 3 + 1] * f22) << 8 | (int)(this.texCols[i19 * 3 + 2] * f22);
												}
											}
										}

										if (i15 >= -1 && i15 < 543)
										{
											i18 = i12 + (i15 + 1) * 32;
											i19 = this.textures[block16.blockID * 3 + 1];
											f20 = world2.getLightBrightness(i9 - 1, i14, i8) * 0.8F + 0.2F;
											int i21 = this.textures[block16.blockID * 3 + 2];
											f22 = world2.getLightBrightness(i9, i14, i8 + 1) * 0.8F + 0.2F;
											float f23;
											if (i12 >= 0)
											{
												f23 = f20 * f17 * 0.6F;
												if (this.zBuf[i18] <= i14 - 1)
												{
													this.zBuf[i18] = i14 - 1;
													this.pixels[i18] = unchecked((int)0xFF000000) | (int)(this.texCols[i19 * 3 + 0] * f23) << 16 | (int)(this.texCols[i19 * 3 + 1] * f23) << 8 | (int)(this.texCols[i19 * 3 + 2] * f23);
												}
											}

											if (i12 < 31)
											{
												f23 = f22 * 0.9F * f17 * 0.4F;
												if (this.zBuf[i18 + 1] <= i14 - 1)
												{
													this.zBuf[i18 + 1] = i14 - 1;
													this.pixels[i18 + 1] = unchecked((int)0xFF000000) | (int)(this.texCols[i21 * 3 + 0] * f23) << 16 | (int)(this.texCols[i21 * 3 + 1] * f23) << 8 | (int)(this.texCols[i21 * 3 + 2] * f23);
												}
											}
										}
									}
								}
							}
						}
					}

					this.postProcess();
					if (isoImageBuffer1.image == null)
					{
						isoImageBuffer1.image = new Image<Bgra32>(32, 544);
					}

					int iter = 0;
                    for(int x = 0; x < isoImageBuffer1.image.Width; x++)
                    {
						for (int y = 0; y < isoImageBuffer1.image.Height; y++)
                        {
							Bgra32 pixel = isoImageBuffer1.image[x, y];
							IntByteUnion union = new IntByteUnion() { byte0 = pixel.R, byte1 = pixel.G, byte2 = pixel.B, byte3 = pixel.A };

							pixels[iter] = union.integer;

							iter++;
                        }
					}

					isoImageBuffer1.rendered = true;
				}
			}
		}

		private void postProcess()
		{
			for (int i1 = 0; i1 < 32; ++i1)
			{
				for (int i2 = 0; i2 < 544; ++i2)
				{
					int i3 = i1 + i2 * 32;
					if (this.zBuf[i3] == 0)
					{
						this.pixels[i3] = 0;
					}

					if (this.waterBuf[i3] > this.zBuf[i3])
					{
						int i4 = this.pixels[i3] >> 24 & 255;
						this.pixels[i3] = ((this.pixels[i3] & 16711422) >> 1) + this.waterBr[i3];
						if (i4 < 128)
						{
							this.pixels[i3] = int.MinValue + this.waterBr[i3] * 2;
						}
						else
						{
							this.pixels[i3] |= unchecked((int)0xFF000000);
						}
					}
				}
			}

		}
	}

}